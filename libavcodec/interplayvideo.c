                                ...
                                AVPacket *avpkt)
{
    ...
    send_buffer = AV_RB8(avpkt->data + 1);
    switch (s->frame_format) {
        case 0x06:
            ...
            s->decoding_map = buf + 4 + 14; /* 14 bits of op data */

            s->video_data_size = AV_RL16(avpkt->data + 2) - s->decoding_map_size - 14;
            bytestream2_init(&s->stream_ptr, buf + 4 + s->decoding_map_size + 14, s->video_data_size);
        ...
        case 0x10:
            ...
            s->video_data_size = AV_RL16(avpkt->data + 2);
            bytestream2_init(&s->stream_ptr, buf + 4, s->video_data_size);

            /* decoding map contains 16 bits of information per 8x8 block */
            s->decoding_map_size = AV_RL16(avpkt->data + 4 + s->video_data_size);
            s->decoding_map = buf + 4 + s->video_data_size + 2;
            bytestream2_init(&s->decoding_map_ptr, s->decoding_map, s->decoding_map_size);

            /* skip map contains 16 bits of information per 15 blocks, ish */
            s->skip_map_size = AV_RL16(avpkt->data + 4 + s->video_data_size + 2 + s->decoding_map_size);
            s->skip_map = buf + 4 + s->video_data_size + 2 + s->decoding_map_size + 2;
