...
static int load_ipmovie_packet(IPMVEContext *s, AVIOContext *pb,
    ...
    if (s->audio_chunk_offset && s->audio_channels && s->audio_bits) {
        ...
        s->frame_format = 0;
        AV_WB8(pkt->data + 1, s->send_buffer);
        s->send_buffer = 0;
        ...
        if (avio_read(pb, pkt->data + 4, s->video_chunk_size) !=
            s->video_chunk_size) {
            av_packet_unref(pkt);
            return CHUNK_EOF;
        }
        ...
        if (s->decode_map_chunk_size) {
            ...
            AV_WL16(pkt->data + 4 + s->video_chunk_size, s->decode_map_chunk_size);
            if (avio_read(pb, pkt->data + 4 + s->video_chunk_size + 2, s->decode_map_chunk_size) !=
        ...
        if (s->skip_map_chunk_size) {
            ...
            AV_WL16(pkt->data + 4 + s->video_chunk_size + 2 + s->decode_map_chunk_size, s->skip_map_chunk_size);
            if (avio_read(pb, pkt->data + 4 + s->video_chunk_size + 2 + s->decode_map_chunk_size + 2, s->skip_map_chunk_size) !=
